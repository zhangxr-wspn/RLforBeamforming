...
class RayleighEnvWithChannel(gym.Env):
    metadata = {
...
#        self.max_distance=3 # maximum distance 3km
        ...
        self.low_fading = -self.high_fading
        ...
        self.observation_space = spaces.Box(low=np.array(np.append([self.low_fading],[self.low_length])),\
                                            high=np.array(np.append([self.high_fading],[self.high_length])), dtype='float32')
...
#        self.state = np.array([newl1_packet, newl2_packet])
        self.state = np.append(self.np_random.randn(2*self.Nt*self.K_User*self.M_Group),np.array([newl1_packet, newl2_packet]))
        
        
        return self.state, -costs, False, {}
###########################pass in the Nt number##################
    def reset(self):
        self.state = np.append(self.np_random.randn(2*self.Nt*self.K_User*self.M_Group),np.array([30,30]))# Initial length
        self.last_u = None
        return self.state
